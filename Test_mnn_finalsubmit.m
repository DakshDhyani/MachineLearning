...
load('network_dnn_alpha_finalsubmit.mat','net');
load('network_dnn_beta_finalsubmit.mat','net1');
load('network_dnn_mach_finalsubmit.mat','net2');
load('network_mnn_finalsubmit.mat','net3');
...
%storing the index to start for in air data(+13000 and -500 is done to get
%the proper in air data.
refer_index_start = S(1)+13000;  %due to flight data error it is +13000
...
%Extracting all the states to have the parameters after
%transformation.(like pitch and roll anngle).
...
%otherwise take it.
to_check = size(lat_acc,1);  %lat_acc is taken as reference as its already 40hz and we need rest also to be of same dimension same as that 
...
%For estimation of beta i.e. Angle of sideslip
...
%the prediction for the alpha
Yprect_alpha = predict(net,arr_in_alpha');
Yprect_alpha = transpose(Yprect_alpha);

%the prediction for the beta
Yprect_beta = predict(net1,arr_in_beta');
Yprect_beta = transpose(Yprect_beta);

%the prediction for the mach number
Yprect_mach = predict(net2,arr_in_mach');
Yprect_mach = transpose(Yprect_mach);

%Testing of Mnn
arr_in_mnn = [lat_acc ,normal_acc , expanded_sined_roll,expanded_cosined_roll,expanded_sined_pitch,expanded_cosined_pitch,roll_rate,pitch_rate,yaw_rate,expanded_velocity_x,expanded_velocity_y,expanded_velocity_z,expanded_H,lie_ram_pos_cmd,rie_ram_pos_cmd,rudder_ram_pos_cmd ,Yprect_alpha,Yprect_beta,Yprect_mach];
arr_in_mnn = normalize(arr_in_mnn);
arr_out_mnn = arr_output;



%Testing the mnn network the mnn network
Yprect_mnn = predict(net3,arr_in_mnn');
Yprect_mnn = transpose(Yprect_mnn);

%plot and see the deviation if any
figure;
plot(Yprect_mnn(:,1) , 'g','LineWidth',1),hold on
plot(arr_out_mnn(:,1),'y','LineWidth',1);hold all;
xlabel('sample_mnn_alpha')
ylabel('values_mnn_alpha')
grid on 
legend("Predicted","original");
title("Performance")

figure;
plot(Yprect_mnn(:,2) , 'g','LineWidth',1),hold on
plot(arr_out_mnn(:,2),'y','LineWidth',1);hold all;
xlabel('sample_mnn_beta')
ylabel('values_mnn_beta')
grid on 
legend("Predicted","original");
title("Performance")

figure;
plot(Yprect_mnn(:,3) , 'g','LineWidth',1),hold on
plot(arr_out_mnn(:,3),'y','LineWidth',1);hold all;
xlabel('sample_mnn_mach')
ylabel('values_mnn_mach')
grid on 
legend("Predicted","original");
title("Performance")
...
figure;
plot(Yprect_alpha' , 'g','LineWidth',1),hold on
plot(arr_out_alpha,'y','LineWidth',1);hold all;
xlabel('sample_alpha')
ylabel('values_alpha')
grid on 
legend("Predicted","original");
...
figure;
plot(Yprect_beta' , 'g','LineWidth',1),hold on
plot(arr_out_beta,'y','LineWidth',1);hold all;
xlabel('sample_beta')
ylabel('values_beta')
grid on 
legend("Predicted","original");
...
figure;
plot(Yprect_mach' , 'g','LineWidth',1),hold on
plot(arr_out_mach,'y','LineWidth',1);hold all;
xlabel('sample_mach')
ylabel('values_mach')
grid on 
legend("Predicted","original");
