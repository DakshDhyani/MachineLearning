...
plot(arr_out_alpha,'b','LineWidth',1);hold all;
xlabel('sample_alpha')
ylabel('values_alpha')
grid on 
...
plot(arr_out_beta,'b','LineWidth',1);hold all;
xlabel('sample_beta')
ylabel('values_beta')
grid on 
legend("Predicted","original");
...
plot(arr_out_mach,'b','LineWidth',1);hold all;
xlabel('sample_mach')
ylabel('values_mach')
grid on 
legend("Predicted","original");
...
%net =  layerGraph([layers;sequenceFoldingLayer('Name','output_folding')]);
...
%Training the mnn network
net3 = trainNetwork(arr_in_mnn',arr_out_mnn',layers,options);
save('network_mnn_finalsubmit','net3');
